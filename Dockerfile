...
RUN apt-get dist-upgrade && apt-get update && \
    apt-get -y install xz-utils unzip jq

RUN apt-get -y install gcc g++ make cmake yasm autopoint gettext libtool
...
# Explicitly set the go build cache
...
COPY Cargo.* ./
COPY gifski.h .
RUN cargo build --features default --release
